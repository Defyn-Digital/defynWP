...
{
    private const REFRESH_HOOK = 'defyn_refresh_site_core';
    ...
    private function setupOwnedSite(): array
    {
        ...
        delete_transient(sprintf('defyn_rl_core_refresh_%d_%d', $userId, $siteId));
    ...
    public function testSuccessSchedulesJobAndReturns202(): void
    {
        ...
        $req = new WP_REST_Request('POST', '/defyn/v1/sites/' . $ctx['siteId'] . '/core/refresh');
        ...
        self::assertSame('core_inventory.refresh_requested', $event['event_type']);
    ...
    public function testOwnerScoped404(): void
    {
        ...
        $req = new WP_REST_Request('POST', '/defyn/v1/sites/' . $siteId . '/core/refresh');
    ...
    public function testSeventhCallReturns429(): void
    {
        ...
        self::assertSame('core.rate_limited', $res->get_data()['error']['code']);
    }
    ...
    public function testCoreRefreshBucketSeparateFromPluginsAndThemesRefresh(): void
    {
        ...
        for ($i = 0; $i < 6; $i++) {
            ...
            $req->set_header('Authorization', 'Bearer ' . $ctx['jwt']);
            $res = rest_do_request($req);
            self::assertSame(202, $res->get_status(), "themes refresh {$i} should succeed");
        }

        // 7th themes refresh should be 429.
        $req = new WP_REST_Request('POST', '/defyn/v1/sites/' . $ctx['siteId'] . '/themes/refresh');
        $req->set_header('Authorization', 'Bearer ' . $ctx['jwt']);
        $res = rest_do_request($req);
        self::assertSame(429, $res->get_status());
        ...
        // But core refresh should still work — separate bucket.
        $req = new WP_REST_Request('POST', '/defyn/v1/sites/' . $ctx['siteId'] . '/core/refresh');
        $req->set_header('Authorization', 'Bearer ' . $ctx['jwt']);
        $res = rest_do_request($req);
        self::assertSame(202, $res->get_status());
