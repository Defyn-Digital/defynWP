 ...
 * @group integration
 *
 * Regression coverage for the P2.6 POST /defyn/v1/overview/sync-all route —
 * pins the cross-cutting wire the SPA depends on but that the controller
 * tests don't exercise:
 *
 *   1. CORS headers fire for this route (Cors middleware namespace check).
 *
 * Mirrors the prior OverviewCorsTest pattern: drive the middleware directly
 * because rest_pre_serve_request fires outside the WP_REST_Request lifecycle
...
final class OverviewSyncAllCorsTest extends AbstractSchemaTestCase
{
    ...
    public function testOptionsPreflightOnSyncAllRouteReturnsCorsHeaders(): void
    {
        ...
        self::assertSame(
            ...
            'OPTIONS preflight must advertise POST so the SPA can preflight the sync-all fan-out',
        );
