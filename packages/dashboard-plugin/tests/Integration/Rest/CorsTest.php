...
final class CorsTest extends WP_UnitTestCase
{
    ...
    /**
     * v0.30.1: /auth/config is a public defyn/v1/* route the SPA fetches BEFORE
     * login, so Cors::apply must set the CORS origin header — without it the SPA's
     * pre-login fetch would be blocked by the browser. Mirrors the /auth/google lock.
     */
    public function testApplyAddsAccessControlAllowOriginHeaderForAuthConfig(): void
    {
        $response = new WP_REST_Response(['google_client_id' => '', 'error' => null], 200);
        $request  = new WP_REST_Request('GET', '/defyn/v1/auth/config');
        $server   = rest_get_server();
