...
/**
 * P6.2 Task 13 — CORS + route-resolution regression for the analytics routes:
 *   - GET  /defyn/v1/sites/{id}/analytics
 *   - POST /defyn/v1/sites/{id}/analytics/refresh
 *   - POST /defyn/v1/sites/{id}/ga4-property
 *
 ...
 * rest_no_route) to prove all three routes actually register — and that
 * /analytics/refresh doesn't collide with /analytics.
 *
 * @group integration
 */
final class AnalyticsCorsTest extends AbstractSchemaTestCase
{
    ...
    {
        return [
            'GET /sites/1/analytics'          => ['GET', '/defyn/v1/sites/1/analytics'],
            'POST /sites/1/analytics/refresh' => ['POST', '/defyn/v1/sites/1/analytics/refresh'],
            'POST /sites/1/ga4-property'      => ['POST', '/defyn/v1/sites/1/ga4-property'],
        ];
    }

    // -------------------------------------------------------------------------
    // Route-resolution — an authed dispatch to a non-owned site resolves to the
    // controller's 404 sites.not_found, NOT WP's rest_no_route. Proves all three
    // new routes actually registered (and that POST /analytics/refresh doesn't
    // collide with GET /analytics).
    // -------------------------------------------------------------------------
    ...
    {
        return [
            'GET /sites/999999/analytics'          => ['GET', '/defyn/v1/sites/999999/analytics'],
            'POST /sites/999999/analytics/refresh' => ['POST', '/defyn/v1/sites/999999/analytics/refresh'],
            'POST /sites/999999/ga4-property'      => ['POST', '/defyn/v1/sites/999999/ga4-property'],
        ];
    }
}
