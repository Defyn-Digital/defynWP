 ...
 * @group integration
 *
 * Regression coverage for the P2.3 POST /defyn/v1/sites/{id}/themes/{slug}/update
 * route — pins the two cross-cutting wires the SPA depends on but that the
 * controller tests don't exercise:
 *
 ...
 *      that covers /sites/{id}/plugins/refresh must cover the new update route.
 *
 * Mirrors P2.1's CorsTest pattern: drive the middleware directly because
...
final class SitesThemesUpdateCorsTest extends WP_UnitTestCase
{
    ...
    public function testCorsHeadersApplyForThemesIndexRoute(): void
    {
        $response = new WP_REST_Response(['ok' => true], 200);
        $request  = new WP_REST_Request('GET', '/defyn/v1/sites/1/themes');
        ...
        self::assertSame(
            ...
            'OPTIONS preflight must be advertised so the SPA can preflight the GET',
        );
        self::assertSame(false, $served, 'apply must return the served bool unchanged');
    }

    public function testCorsHeadersApplyForThemesUpdateRoute(): void
    {
        $response = new WP_REST_Response(['ok' => true], 200);
