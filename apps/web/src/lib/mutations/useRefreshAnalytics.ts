...
export function useRefreshAnalytics(siteId: number) {
  const queryClient = useQueryClient();
  const preScanRef = useRef<string | null>(null);
  const [isPolling, setIsPolling] = useState(false);

  const query = useSiteAnalytics(siteId, { refetchInterval: isPolling ? 5_000 : false });
  ...
  // Hard timeout at 60s to prevent polling forever if the refresh never completes.
  useEffect(() => {
    if (!isPolling) return;
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return apiClient.post<{ scheduled: boolean; site_id: number }>(
        `/sites/${siteId}/analytics/refresh`,
      );
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['siteAnalytics', siteId] });
      setIsPolling(true);
    },
  });
